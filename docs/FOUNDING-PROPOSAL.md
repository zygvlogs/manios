...
**M6 achieved**: ATA storage — see
[docs/milestones/M6-ata-storage.md](milestones/M6-ata-storage.md).
...
without breaking old binaries silently. *(Implemented at M9: each
executable carries an ELF note with its ABI version, which the loader
checks; see [the M9 notes](milestones/M9-libc-shell.md).)*
