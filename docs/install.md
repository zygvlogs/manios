...
qemu-system-i386 -cpu 486 -m 32 -cdrom manios-0.15.0.iso -boot d -hda disk.img
```
...
The network works with VirtualBox's default card, the PCnet-FAST III,
and with the PCnet-PCI II and the three Intel PRO/1000 cards; not with
"Paravirtualized Network (virtio-net)". (The tests run the PCnet-PCI II
and the PRO/1000 MT Desktop, as QEMU emulates them; the FAST III is
programmed the same way, and the two Server cards are of the same
family, but those three are untested so far.) Without `ip=` on the boot line
ManiOS asks for its address by DHCP, which VirtualBox's NAT (the
default), NAT Network and Host-only networks answer; `cat /dev/net`
shows what it got, and in the kernel monitor (`exit` from the shell)
`ping 10.0.2.2` reaches NAT's gateway. For a cluster of ManiOS machines
(ADR-0005), put them on one "Internal Network" -- it has no DHCP server
-- and give each its own `ip=` (and the same `key=`).
...
| `ip=ADDR/PREFIX` | the network address, e.g. `ip=10.0.0.5/24`; without `ip=` (or with `ip=dhcp`), ManiOS asks a DHCP server, and meanwhile uses 10.0.2.15/24 via 10.0.2.2 (QEMU's and VirtualBox's NAT) |
...
```
sudo dd if=manios-0.15.0.iso of=/dev/sdX bs=1M conv=fsync
```
...
ManiOS 0.15.0 will be installed on ata0 (512 MiB).
...
ManiOS 0.15.0 is installed on ata0. Remove the CD and restart the machine.
```
